(() => {
  ...
  ready(()=>{
    ...
    function audit(legs,variant,isSgp){
      ...
      for(let i=0;i<legs.length;i++)for(let j=i+1;j<legs.length;j++){if(sel.strictIncompatible?.(legs[i],legs[j]))return{pass:false,reason:'Conflicting legs'};const c=corr(legs[i],legs[j]);if(c<0)return{pass:false,reason:'Negative correlation'};total+=c;if(c>=3){strong++;degree[i]++;degree[j]++;}}
      ...
      if(cavg<80)return{pass:false,reason:'Parlay confidence below 80'};
    ...
    function bestFallback(game,count,risk,variant){
      ...
      let best=null,bestScore=-Infinity;const chosen=[];
      function walk(start){if(chosen.length===count){const a=audit(chosen,variant,true);if(!a.pass)return;const score=a.cavg+a.corr*1.5+chosen.reduce((s,l)=>s+value(l,variant),0)/count*.2;if(score>bestScore){bestScore=score;best={legs:[...chosen],audit:a}}return}for(let i=start;i<pool.length;i++){chosen.push(pool[i]);walk(i+1);chosen.pop()}}
      walk(0);if(!best||typeof packageParlay!=='function')return null;const p=packageParlay(best.legs,variant,true,{scriptName:'Connected high-confidence SGP',thesis:'Qualified legs connected by at least one strong game-script correlation anchor; no contradictory relationships.'});if(p){p.corr=best.audit.corr;p.score=Math.min(Number(p.score)||100,Math.round(best.audit.cavg));p.logicTier='connected-fallback'}return p;
    }
    const prev=window.buildSgp;
    if(typeof prev==='function')window.buildSgp=function(game,count,risk,variant,previous=[]){let p=prev(game,count,risk,variant,previous);if(p?.legs?.length&&audit(p.legs,variant,true).pass)return p;p=bestFallback(game,count,risk,variant);if(p)return p;window.NFL_SGP_LAST_FAILURE={gameId:game?.id,count,variant,reason:'No combination cleared confidence, value and connected-correlation gates'};return null};
    window.NFL_FINAL_LOGIC={connectedAudit:(l,v,s)=>audit(l,v,s).pass,audit,bestFallback};
  });
})();