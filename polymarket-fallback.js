(() => {
  const CACHE_KEY='nflParlayPolymarketFallback:v3';
  ...
  function sourceQuality(m,prob){
    let q=50;
    const liquidity=num(m.liquidityNum,m.liquidity,m.liquidityClob);
    const volume=num(m.volumeNum,m.volume,m.volume24hr,m.volume1wk);
  ...
  function addMarket(markets,leg,m){
    ...
    const key=[leg.type,leg.team,leg.side||'',leg.point??''].join('|');
    const idx=markets.findIndex(x=>[x.type,x.team,x.side||'',x.point??''].join('|')===key);
    if(idx<0)markets.push(enriched);else if((markets[idx].sourceQuality||0)<quality)markets[idx]=enriched;
  }
  function plausiblePoint(type,point){if(!Number.isFinite(point))return false;if(type==='spreads')return Math.abs(point)<=20;if(type==='totals')return point>=25&&point<=75;return true;}
  function normalizeEvent(event){
    let teams=findTeams(event.title||'');
    if(teams.length!==2){const eventText=[event.title,event.subtitle,event.slug,event.seriesSlug].filter(Boolean).join(' ');teams=[...new Set(findTeams(eventText))];}
    ...
    const ranked=markets.sort((a,b)=>(b.sourceQuality||0)-(a.sourceQuality||0));
  ...
  async function fetchGames(){
    ...
    try{const tag=await getJson('https://gamma-api.polymarket.com/tags/slug/nfl');if(tag?.id){const url=new URL('https://gamma-api.polymarket.com/events');url.searchParams.set('tag_id',tag.id);url.searchParams.set('active','true');url.searchParams.set('closed','false');url.searchParams.set('limit','100');url.searchParams.set('order','startDate');url.searchParams.set('ascending','true');const raw=await getJson(url);events=Array.isArray(raw)?raw:(raw.events||[]);}}catch(err){console.warn('Polymarket NFL tag lookup failed',err);}
    if(!events.length){for(const q of ['NFL','National Football League']){try{const url=new URL('https://gamma-api.polymarket.com/search');url.searchParams.set('q',q);url.searchParams.set('limit_per_type','100');url.searchParams.set('keep_closed_markets','0');url.searchParams.set('search_profiles','false');const raw=await getJson(url);events=raw.events||[];if(events.length)break;}catch(err){console.warn('Polymarket search failed',q,err);}}}
  ...
  function markSource(games){const note=document.querySelector('.source-note');if(note)note.textContent='Caesars = primary lines & props • Polymarket = filtered fallback implied prices • ESPN/nflverse = context';const p=document.getElementById('polymarketStatus');if(p)p.textContent=games.length?'Active • quality filtered':'Unavailable';}
  async function loadFallback(){try{setStatus('Loading quality-filtered Polymarket NFL markets…');const games=await fetchGames();if(!games.length)throw new Error('No quality NFL markets returned by Polymarket');state.games=games;state.propsLoaded.clear();hydrateGames();markSource(games);const legs=document.getElementById('legsSelect');if(state.mode==='sgp'&&legs&&Number(legs.value)>2)legs.value='2';const marketCount=games.reduce((n,g)=>n+g.markets.length,0);setStatus(`Polymarket fallback • ${games.length} NFL games • ${marketCount} quality-filtered markets`);await generate();const title=document.getElementById('resultsTitle');if(title&&state.mode==='sgp')title.textContent='Polymarket fallback • quality-filtered team markets';window.NFL_PARLAY_DAILY_PICKS?.refresh?.();window.NFL_BANKROLL?.refresh?.();return true;}catch(err){console.warn('Polymarket fallback unavailable',err);markSource([]);state.games=structuredClone(demoGames);hydrateGames();setStatus('Live sources unavailable — demo data active');await generate();return false;}}
  ...
  window.NFL_POLYMARKET_FALLBACK={load:loadFallback,fetchGames,clearCache:()=>{['nflParlayPolymarketFallback:v1','nflParlayPolymarketFallback:v2',CACHE_KEY].forEach(k=>localStorage.removeItem(k));}};
  ['nflParlayPolymarketFallback:v1','nflParlayPolymarketFallback:v2'].forEach(k=>localStorage.removeItem(k));
