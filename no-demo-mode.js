(() => {
  function clearLiveState(message='No live markets available'){
    ...
    try{
      ...
      const results=document.getElementById('results');if(results)results.innerHTML='<div class="empty">Live Caesars, Kalshi, or Polymarket data is required. No demo or placeholder markets are used.</div>';
  ...
  function hasLiveGames(){try{return (state.games||[]).some(g=>['Kalshi','Polymarket'].includes(g?.dataSource)||(!String(g?.id||'').startsWith('demo-')))}catch{return false}}
  async function fallbackChain(){
    ...
    clearLiveState('Kalshi unavailable — trying Polymarket…');
    try{if(await window.NFL_POLYMARKET_FALLBACK?.load?.())return true}catch(e){console.warn('Polymarket fallback failed',e)}
    clearLiveState('No live Caesars, Kalshi, or Polymarket markets available');
    return false;
  }
  const caesarsLoad=async()=>{
    if(!state.apiKey)return fallbackChain();
    try{
    ...
    }catch(err){console.warn('Caesars unavailable',err);return fallbackChain()}
  };
  loadData=caesarsLoad;
  const originalGenerate=generate;
  generate=async function(...args){if(!hasLiveGames()){clearLiveState(state.apiKey?'Waiting for Caesars/Kalshi/Polymarket…':'Waiting for Kalshi/Polymarket…');return}return originalGenerate.apply(this,args)};
  window.NFL_NO_DEMO={clear:clearLiveState,hasLiveGames,load:caesarsLoad,fallback:fallbackChain};
