...
function evaluate({marketP,odds,signal=0,coverage=.25,quality=60,uncertainty=.25,sample=0}={}){const modelP=probability({marketP,signal,coverage,uncertainty}),price=Number.isFinite(odds)&&Math.abs(odds)>=100?odds:(marketP>=.5?-100*marketP/(1-marketP):100*(1-marketP)/marketP),edge=modelP-implied(price);return{marketP,modelP,edge,ev:ev(modelP,price),confidence:confidence({coverage,quality,uncertainty,sample}),quality,coverage,uncertainty,odds:price}}
